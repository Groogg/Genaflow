...
with dag:
    check_for_empty_folder_report_download = PythonOperator(
        task_id='check_for_empty_folder_report_download',
    ...
    check_for_empty_folder_shp_update_store = PythonOperator(
        task_id='check_for_empty_folder_shp_update_store',
        python_callable=extract.folder_count_sensor,
        op_kwargs={'folder_path': PATHS['shp_update_folder'],
                   'wait': False,
                   'nbr_of_files': 0}
    )

    stores_selenium_job = PythonOperator(
        task_id='stores_selenium_job',
        python_callable=selenium.shp_stores_selenium,
        op_kwargs={'folder_path': PATHS["shp_update_folder"],
    ...
    update_store_master_file = PythonOperator(
        task_id="update_store_master_file",
        python_callable=transform_load.update_shp_stores,
        op_kwargs={
            'master_folder_path': PATHS['shp_master_folder'],
            'update_folder_path': PATHS['shp_update_folder']
        }
    )

    report_selenium_job = PythonOperator(
        task_id='report_selenium_job',
        python_callable=selenium.shp_selenium,
        op_kwargs={'folder_path': PATHS['download_report_folder'],
                   "driver_path": PATHS['chrome_driver_path'],
                   'url': CONFIG['online_portal']['url'],
                   'html_username_attribute': CONFIG['online_portal']['html_username_attribute'],
    ...
    transform_load = PythonOperator(
        ...
        python_callable=transform_load.shp_transform_load,
        op_kwargs={'folder_path': PATHS['download_report_folder'],
                   ...
                   "store_master_file_path": PATHS['shp_master_folder'] + '/shp_master_store_list.csv'}
    )
    ...
    check_for_empty_folder_report_download >> check_for_empty_folder_shp_update_store \
    >> stores_selenium_job >> update_store_master_file >> report_selenium_job \
    >> wait_for_file_to_appear >> check_if_new_report >> rename_report \
    >> transform_load >> archive_report >> update_report_tracker >> ONE_FAILED__move_report
